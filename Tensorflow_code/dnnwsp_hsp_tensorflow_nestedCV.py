...
final_directory = os.path.join(current_directory, r'results_CV_%s'%(mode))
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True, log_device_placement=True)) as sess:
        ...
        for outer in range(k_folds):
            ...
            for tg_hspset in tg_hspset_list:
                ...
                for inner in outer_train_list:
                    ...
                    if autoencoder==False:      
                        ...
                        # make a new 'results' directory in the current directory
                        current_directory = os.getcwd()
                        final_directory = os.path.join(current_directory, r'results_CV_%s/outer%d/tg_%s/inner%d'%(mode,outer+1,tghsp,inner))
                        if not os.path.exists(final_directory):
                            os.makedirs(final_directory) 
            ...
            if autoencoder==False: 
                ...
                current_directory = os.getcwd()
                final_directory = os.path.join(current_directory, r'results_CV_%s/outer%d_selected_%s'%(mode,outer+1,selectedhsp))
                if not os.path.exists(final_directory):
                    os.makedirs(final_directory) 
        ...
        f = open(final_directory+"/final_results.txt",'w') 
